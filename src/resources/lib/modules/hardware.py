...
class hardware:
    ...
    def load_values(self):
        try:
            ...
            if os.path.exists("/storage/.config/edid-dmp.bin"):
        ...
        try:
            ...
            if not listItem == None:
                ...
                if self.struct['display']['settings']['dump_edid']['value'] == '1':
                    self.struct['display']['settings']['dump_edid']['name'] = 32540
                    self.struct['display']['settings']['dump_edid']['InfoText'] = 915
                    if not os.path.exists("/storage/.config/edid-dmp.bin"):
                        subprocess.call("cat /sys/class/amhdmitx/amhdmitx0/rawedid > /storage/.config/edid-dmp.bin", shell=True)
                else:
                    self.struct['display']['settings']['dump_edid']['name'] = 32539
                    self.struct['display']['settings']['dump_edid']['InfoText'] = 914
                    if os.path.exists("/storage/.config/edid-dmp.bin"):
                        subprocess.call("rm -rf /storage/.config/edid-dmp.bin", shell=True)
